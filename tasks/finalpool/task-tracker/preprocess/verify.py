"""
Verify that task-tracker preprocess completed correctly.

Checks:
  1. GitHub repo 'BenchTasksCollv3' exists under the authenticated user
  2. The repo has commits (is not empty)
  3. The duplicated Notion page ID file was created
"""
...
def verify() -> bool:
    ...
    # 3. Check duplicated Notion page ID file
    task_root = Path(current_dir).parent
    page_id_file = task_root / "files" / "duplicated_page_id.txt"
    if page_id_file.exists():
        page_id = page_id_file.read_text().strip()
        if page_id:
            print(f"[verify] OK: Duplicated Notion page ID file exists (ID: {page_id[:12]}...)")
        else:
            print(f"[verify] FAIL: Duplicated page ID file is empty")
            ok = False
    else:
        print(f"[verify] FAIL: {page_id_file} does not exist")
        ok = False
